...
function createArticleCard(article) {
  ...
  // Create a container for title and category pill
  ...
  // Add a black edge to the pill
  tag.style.border = "1px solid black";
  tag.style.padding = "2px 6px";
  tag.style.cursor = "pointer";

  // Clicking the pill applies the filter
...
// Author
  ...
  playlistDiv.classList.add('row1-left'); // using row1-left as the container class
  ...
  if (article.songs && article.songs.length > 0) {
    article.songs.forEach(song => {
      if (song.link && song.link.includes('bandcamp.com/album')) {
        ...
        const albumName = lastSegment.replace(/\?from.*$/, '').replace(/-/g, ' ');
  ...
  uniqueDailyLinks.slice(0, 3).forEach(dl => {
    ...
    a.addEventListener('click', (e) => {
      ...
      if (dailyArticle) {
        ...
        // Optionally adjust currentIndex or any other state as needed
...
if (window.innerWidth <= 768) {
  ...
  row1Right.appendChild(playlistDiv);
